		...
		case I2CSlave::ReadAddressed: //MASTER IS READING AT THE SLAVE ADRESS
		{
			switch(i2cslave) {
				...
				case 1:
					...
					i2cslave.write(tab_cord , sizeof(tab_cord[0]));
				break;
			}
			i2cslave.write(dataAdress , sizeof(dataArray[0]));

			pc.printf("writen %d bytes: %f\n", sizeof(dataArray[0]), *( (float*) dataAdress));
